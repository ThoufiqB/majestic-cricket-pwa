...
import { adminUpdateAttendance, getAdminEventAttendance, adminDeleteAttendee } from "./services";
...
export function useAdminEventDetails(eventId: string) {
  ...
  const load = useCallback(async () => {
    ...
    const { event: ev, rows: r } = await getAdminEventAttendance(eventId);
  ...
  const setAttended = useCallback(
    async (player_id: string, attended: boolean) => {
      ...
      try {
        await adminUpdateAttendance(eventId, player_id, { attended });
        await load();
      } catch (e: any) {
  ...
  const setPaidStatus = useCallback(
    async (player_id: string, paid_status: PaidStatus) => {
      ...
      try {
        await adminUpdateAttendance(eventId, player_id, { paid_status });
        await load();
      } catch (e: any) {
        setErr(String(e?.message || e));
      } finally {
  ...
  const bulkMarkAttendedYes = useCallback(async () => {
    ...
    try {
      ...
      for (const r of yesNotAttended) {
        await adminUpdateAttendance(eventId, r.player_id, { attended: true });
      }
  ...
  const deleteAttendee = useCallback(
    async (player_id: string, playerName: string) => {
      ...
      try {
        await adminDeleteAttendee(eventId, player_id);
        await load();
